...
must_haves:
  truths:
    - "User sees visual progress through onboarding with completed, active, and pending step indicators"
    - "User password must include a special character to be accepted"
    - "User receives clear feedback when organization name format is invalid"
    - "User receives immediate feedback on organization name availability"
  artifacts:
