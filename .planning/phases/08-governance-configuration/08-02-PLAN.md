...
must_haves:
  truths:
    ...
    - "User can set approval email address"
...
Purpose: Complete all GOV requirements by connecting the existing form to the backend, adding the missing approvalEmail field, and using react-hook-form for proper validation.
...
2. Define local form schema (for UI validation):
   ...
   const governanceFormSchema = z.object({
     ...
     approvalEmail: z.string().email().or(z.literal("")),
...
7. ADD missing approvalEmail field (GOV-03):
   - Place between Low ROI Threshold and Dashboard Refresh
   - Label: "Approval Email"
   - Description: "High-cost meetings will be routed to this email for approval"
   - Use Input component with type="email"
   - Register with react-hook-form
   - Show validation error if invalid email format

8. Update existing fields to use react-hook-form:
...
9. Replace handleNext with handleSubmit(onSubmit):
...
- approvalEmail field is present (GOV-03)
- All 5 governance fields editable (GOV-01 through GOV-05)
...
3. Manual test flow:
   ...
   - Enter approval email: test@example.com
...
<success_criteria>
- All 6 governance requirements (GOV-01 through GOV-06) satisfied
- approvalEmail field visible and functional
- Form validates input (email format, number ranges)
