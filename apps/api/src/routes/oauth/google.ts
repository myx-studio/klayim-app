 ...
 * Google OAuth Routes
 *
 * Handles Google Calendar OAuth authorization flow:
 ...
 */
googleOAuth.get("/authorize", authMiddleware, async (c) => {
  try {
    ...
    const authUrl = googleCalendarService.getAuthUrl(JSON.stringify(state));
...
googleOAuth.get("/callback", async (c) => {
  ...
  let state: GoogleOAuthState | null = null;
  try {
    if (stateParam) {
  ...
  if (error) {
    ...
    errorRedirect.searchParams.set("provider", "google_calendar");
  ...
  try {
    ...
    successRedirect.searchParams.set("provider", "google_calendar");
