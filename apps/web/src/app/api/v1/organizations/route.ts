...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error instanceof ApiError) {
      ...
      return NextResponse.json(
        { success: false, error: errorData?.error || "Failed to create organization" },
        { status: error.status }
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await api<ApiResponse<{ organizations: Organization[] }>>(
      "/organizations",
      {
        method: "GET",
        headers: {
    ...
    if (error instanceof ApiError) {
      const errorData = error.data as { error?: string } | undefined;
      return NextResponse.json(
        { success: false, error: errorData?.error || "Failed to fetch organizations" },
        { status: error.status }
      );
    }
